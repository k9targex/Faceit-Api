...
export const StatsPage = () => {
  ...
  useEffect(() => { 
    axios
    .get("http://localhost:8080/auth/secure")
    .catch((error) => {
  ...
  const player_name = Cookies.get('player_name');
        ...
        .then((response) => {
            ...
            const lvl = response.data.skillLevel;
                    Cookies.set('avatar', avatar);
                     Cookies.set('player_name', nickname);
  ...
  const handleLike = async (event) => {
    event.preventDefault(); 
    const data = {
        nickname: Cookies.get('player_name')
    };
...
useEffect(() => {
    ...
    const nickname = Cookies.get('player_name');
    const nicknamesString = Cookies.get('favoritePlayers');
    const nicknames = nicknamesString ? JSON.parse(nicknamesString) : [];
    ...
    >
      ...
      <div className='main-info'>
        ...
        <div className='userInfo'>
          ...
          <p className='userName'>{player_name}</p>
        </div>
